...
/**
 * Description of _TypeIntervention
 *
 * @author user
 */
...
class _TypeIntervention extends DataBaseConnection {

    function __construct() {
        parent::__construct();
    }

    //Recuperer la liste des TypeIntervention
...
// Query SQL
            $query = "SELECT * FROM TypeIntervention";
...
// Boucler sur les resultats            
            while ($data = $response->fetch()) {
                ...
                if (isset($data['id'])) {
                    $object = new TypeIntervention($data['id']);
                } else {
                    // ERROR
                    return null;
                }
    ...
    function getTypeInterventionByID($id) {
        try {
            // INITIALISER LA CONNEXION BDD
            if (is_null(parent::getBdd())) {
                parent::__construct();
            ...
            $query = "SELECT * FROM TypeIntervention WHERE ID = :id";
