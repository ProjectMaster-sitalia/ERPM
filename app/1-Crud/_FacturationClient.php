...
/**
 * Description of _FacturationClient
 *
 * @author kzexo
 */
require_once 'DataBaseConnection.php';
require_once './2-Model/FacturationClient.php';

class _FacturationClient extends DataBaseConnection {

    function __construct() {
        parent::__construct();
    }

    //Recuperer la liste des FacturationClients
    function getAllFacturationClients() {
...
// Boucler sur les resultats            
            while ($data = $response->fetch()) {
                ...
                if (isset($data['id'])) {
                    $object = new FacturationClient($data['interventionID'], $data['artisanID']);
                } else {
                    // ERROR
                    return null;
                }
    ...
    function getFacturationClientByID($interventionID, $artisanID) {
        try {
            // INITIALISER LA CONNEXION BDD
            if (is_null(parent::getBdd())) {
                parent::__construct();
